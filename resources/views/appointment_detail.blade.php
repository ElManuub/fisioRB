<x-app-layout>
  ...
  <div class="w-full max-w-screen-lg mx-auto flex justify-center p-2">
    ...
    <form class="w-full sm:w-1/2 lg:w-1/2 bg-white rounded-md p-4 sm:m-4 shadow-md mx-auto" id="appointment" action="{{ route('appointment.total')}}" method="post">
      ...
      <!-- fecha -->
      <div class="relative z-0 w-full mb-5 group">
        <input type="date" name="date" value="{{ $patient['date'] }}" id="date" class="block py-2.5 px-0 w-full text-sm text-gray-900 bg-transparent border-0 border-b-2 border-gray-300 appearance-none focus:outline-none focus:ring-0 focus:border-blue-600 peer" placeholder=" " required readonly />
      ...
      <div class="relative z-0 w-full mb-5 group">
        ...
        <div class="bg-white p-4 border border-gray-300 rounded-md mt-2">
          <div class="flex items-center">
            <input id="laser" type="checkbox" value="500" class="text-blue-600 focus:ring-blue-600" />
            <label for="laser" class="ml-2 text-sm text-gray-700">Laser ($500)</label>
          </div>
          <div class="flex items-center">
            <input id="mep_columna" type="checkbox" value="1000" class="text-blue-600 focus:ring-blue-600" />
            <label for="mep_columna" class="ml-2 text-sm text-gray-700">Mep en Columna ($1000)</label>
          </div>
          <div class="flex items-center">
            <input id="mep_other" type="checkbox" value="500" class="text-blue-600 focus:ring-blue-600" />
            <label for="mep_other" class="ml-2 text-sm text-gray-700">Mep en otra extremidad ($500)</label>
          </div>
          <div class="flex items-center">
            <input id="infiltracion_rodilla" type="checkbox" value="800" class="text-blue-600 focus:ring-blue-600" />
            <label for="infiltracion_rodilla" class="ml-2 text-sm text-gray-700">Infiltración rodilla ($800)</label>
          </div>
          <div class="flex items-center">
            <input id="infiltracion_columna" type="checkbox" value="1000" class="text-blue-600 focus:ring-blue-600" />
            <label for="infiltracion_columna" class="ml-2 text-sm text-gray-700">Infiltración culumna ($1000)</label>
          </div>
          <div class="flex items-center">
            <input id="tape" type="checkbox" value="150" class="text-blue-600 focus:ring-blue-600" />
            <label for="tape" class="ml-2 text-sm text-gray-700">Tape ($150)</label>
          </div>
          <div class="flex items-center">
            <input id="ondas_choque" type="checkbox" value="800" class="text-blue-600 focus:ring-blue-600" />
            <label for="ondas_choque" class="ml-2 text-sm text-gray-700">Ondas de choque ($800)</label>
          </div>
      ...
      <!-- Extra -->
      <div class="relative z-0 w-full mb-5 group">
        <input type="text" name="extra" id="extra" class="block py-2.5 px-0 w-full text-sm text-gray-900 bg-transparent border-0 border-b-2 border-gray-300 appearance-none focus:outline-none focus:ring-0 focus:border-blue-600 peer" placeholder=""/>
        <label for="extra" class="peer-focus:font-medium absolute text-sm text-gray-500 duration-300 transform -translate-y-6 scale-75 top-3 -z-10 origin-[0] peer-focus:start-0 rtl:peer-focus:translate-x-1/4 peer-focus:text-blue-600 peer-placeholder-shown:scale-100 peer-placeholder-shown:translate-y-0 peer-focus:scale-75 peer-focus:-translate-y-6">Gasto Extra</label>
      </div>

      <!-- Total -->
      <div class="relative z-0 w-full mb-5 group">
        <input type="text" name="total" value="0" id="total" class="block py-2.5 px-0 w-full text-sm text-gray-900 bg-transparent border-0 border-b-2 border-gray-300 appearance-none focus:outline-none focus:ring-0 focus:border-blue-600 peer" placeholder=" " required readonly />
        <label for="total" class="peer-focus:font-medium absolute text-sm text-gray-500 duration-300 transform -translate-y-6 scale-75 top-3 -z-10 origin-[0] peer-focus:start-0 rtl:peer-focus:translate-x-1/4 peer-focus:text-blue-600 peer-placeholder-shown:scale-100 peer-placeholder-shown:translate-y-0 peer-focus:scale-75 peer-focus:-translate-y-6"></label>
      </div>

      <!-- Enviar -->
      <div class="relative z-0 w-full mb-5 group">
        <x-primary-button type="submit" id="enviar">Guardar</x-primary-button>
      </div>
    </form>
    @endif
  </div>
